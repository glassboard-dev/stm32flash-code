...
static stm32_err_t stm32_get_ack_timeout(const stm32_t* stm, time_t timeout)
{
    ...
    if (!(port->flags & PORT_RETRY)) timeout = 0;
    ...
    do {
        ...
        if (byte == STM32_ACK) return STM32_ERR_OK;
        if (byte == STM32_NACK) return STM32_ERR_NACK;
        if (byte != STM32_BUSY) {
...
static stm32_err_t stm32_send_command_timeout(const stm32_t* stm, const uint8_t cmd, time_t timeout)
{
    ...
    if (s_err == STM32_ERR_OK) return STM32_ERR_OK;
...
static stm32_err_t stm32_resync(const stm32_t* stm)
{
    ...
    while (t1 < t0 + STM32_RESYNC_TIMEOUT) {
        ...
        if (ack == STM32_NACK) return STM32_ERR_OK;
        time(&t1);
    }
    return STM32_ERR_UNKNOWN;
}
...
static stm32_err_t stm32_guess_len_cmd(const stm32_t* stm, uint8_t cmd, uint8_t* data, unsigned int len)
{
    ...
    if (stm32_send_command(stm, cmd) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
    if (port->flags & PORT_BYTE) {
        /* interface is UART-like */
        p_err = port->read(port, data, 1);
        if (p_err != PORT_ERR_OK) return STM32_ERR_UNKNOWN;
        len = data[0];
    ...
    if (p_err == PORT_ERR_OK && len == data[0]) return STM32_ERR_OK;
    if (p_err != PORT_ERR_OK) {
        /* restart with only one byte */
        if (stm32_resync(stm) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
        if (stm32_send_command(stm, cmd) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
        p_err = port->read(port, data, 1);
        if (p_err != PORT_ERR_OK) return STM32_ERR_UNKNOWN;
    }
...
static stm32_err_t stm32_send_init_seq(const stm32_t* stm)
{
    ...
    p_err = port->read(port, &byte, 1);
    if (p_err == PORT_ERR_OK && byte == STM32_ACK) return STM32_ERR_OK;
    ...
    p_err = port->read(port, &byte, 1);
    if (p_err == PORT_ERR_OK && byte == STM32_NACK) return STM32_ERR_OK;
    if (p_err == PORT_ERR_TIMEDOUT) {
...
stm32_t* stm32_init(struct port_interface* port, const char init)
{
    ...
    if ((port->flags & PORT_CMD_INIT) && init)
        if (stm32_send_init_seq(stm) != STM32_ERR_OK) return NULL;
    ...
    if (port->read(port, buf, len) != PORT_ERR_OK) return NULL;
    ...
    if (stm32_guess_len_cmd(stm, STM32_CMD_GET, buf, len) != STM32_ERR_OK) return NULL;
    len = buf[0] + 1;
...
stm32_err_t stm32_read_memory(const stm32_t* stm, uint32_t address, uint8_t data[], unsigned int len)
{
    ...
    if (stm32_send_command(stm, stm->cmd->rm) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
    ...
    if (port->write(port, buf, 5) != PORT_ERR_OK) return STM32_ERR_UNKNOWN;
    if (stm32_get_ack(stm) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;

    if (stm32_send_command(stm, len - 1) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;

    if (port->read(port, data, len) != PORT_ERR_OK) return STM32_ERR_UNKNOWN;
...
stm32_err_t stm32_write_memory(const stm32_t* stm, uint32_t address, const uint8_t data[], unsigned int len)
{
    ...
    if (stm32_send_command(stm, stm->cmd->wm) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
    ...
    if (port->write(port, buf, aligned_len + 2) != PORT_ERR_OK) return STM32_ERR_UNKNOWN;

    s_err = stm32_get_ack_timeout(stm, STM32_BLKWRITE_TIMEOUT);
    if (s_err != STM32_ERR_OK) {
        if ((port->flags & PORT_STRETCH_W) && stm->cmd->wm != STM32_CMD_WM_NS) stm32_warn_stretching("write");
        return STM32_ERR_UNKNOWN;
    }
    return STM32_ERR_OK;
}
...
stm32_err_t stm32_wunprot_memory(const stm32_t* stm)
{
    ...
    if (stm32_send_command(stm, stm->cmd->uw) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
    ...
    if (s_err != STM32_ERR_OK) {
        if ((port->flags & PORT_STRETCH_W) && stm->cmd->uw != STM32_CMD_UW_NS) stm32_warn_stretching("WRITE UNPROTECT");
        return STM32_ERR_UNKNOWN;
    }
    return STM32_ERR_OK;
}
...
stm32_err_t stm32_wprot_memory(const stm32_t* stm)
{
    ...
    if (stm32_send_command(stm, stm->cmd->wp) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
    ...
    if (s_err != STM32_ERR_OK) {
        if ((port->flags & PORT_STRETCH_W) && stm->cmd->wp != STM32_CMD_WP_NS) stm32_warn_stretching("WRITE PROTECT");
        return STM32_ERR_UNKNOWN;
    }
    return STM32_ERR_OK;
}
...
stm32_err_t stm32_runprot_memory(const stm32_t* stm)
{
    ...
    if (stm32_send_command(stm, stm->cmd->ur) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
...
stm32_err_t stm32_readprot_memory(const stm32_t* stm)
{
    ...
    if (stm32_send_command(stm, stm->cmd->rp) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
    ...
    if (s_err != STM32_ERR_OK) {
        if ((port->flags & PORT_STRETCH_W) && stm->cmd->rp != STM32_CMD_RP_NS) stm32_warn_stretching("READOUT PROTECT");
        return STM32_ERR_UNKNOWN;
    }
    return STM32_ERR_OK;
}
...
static stm32_err_t stm32_mass_erase(const stm32_t* stm)
{
    ...
    if (stm->cmd->er == STM32_CMD_ER) {
        ...
        if (s_err != STM32_ERR_OK) {
            if (port->flags & PORT_STRETCH_W) stm32_warn_stretching("mass erase");
            return STM32_ERR_UNKNOWN;
        }
        return STM32_ERR_OK;
    }
    ...
    if (s_err != STM32_ERR_OK) {
        ...
        if ((port->flags & PORT_STRETCH_W) && stm->cmd->er != STM32_CMD_EE_NS) stm32_warn_stretching("mass erase");
        return STM32_ERR_UNKNOWN;
    }
    return STM32_ERR_OK;
}
...
static stm32_err_t stm32_pages_erase(const stm32_t* stm, uint32_t spage, uint32_t pages)
{
    ...
    if (stm->cmd->er == STM32_CMD_ER) {
        ...
        if (!buf) return STM32_ERR_UNKNOWN;
        ...
        if (s_err != STM32_ERR_OK) {
            ...
            if (port->flags & PORT_STRETCH_W) stm32_warn_stretching("erase");
            return STM32_ERR_UNKNOWN;
        }
        return STM32_ERR_OK;
    }
    ...
    if (s_err != STM32_ERR_OK) {
        ...
        if ((port->flags & PORT_STRETCH_W) && stm->cmd->er != STM32_CMD_EE_NS) stm32_warn_stretching("erase");
        return STM32_ERR_UNKNOWN;
    }
...
stm32_err_t stm32_erase_memory(const stm32_t* stm, uint32_t spage, uint32_t pages)
{
    ...
    if (pages == STM32_MASS_ERASE) {
         ...
         */
        if (!(stm->dev->flags & F_NO_ME)) return stm32_mass_erase(stm);
    ...
    while (pages) {
        ...
        if (s_err != STM32_ERR_OK) return s_err;
        spage += n;
        pages -= n;
    }
    return STM32_ERR_OK;
                                      ...
                                      uint32_t code_size)
{
    ...
    mem = malloc(length);
    if (!mem) return STM32_ERR_UNKNOWN;
...
stm32_err_t stm32_go(const stm32_t* stm, uint32_t address)
{
    ...
    if (stm32_send_command(stm, stm->cmd->go) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
...
stm32_err_t stm32_crc_memory(const stm32_t* stm, uint32_t address, uint32_t length, uint32_t* crc)
{
    ...
    if (stm32_send_command(stm, stm->cmd->crc) != STM32_ERR_OK) return STM32_ERR_UNKNOWN;
    ...
    if (port->read(port, buf, 5) != PORT_ERR_OK) return STM32_ERR_UNKNOWN;

    if (buf[4] != (buf[0] ^ buf[1] ^ buf[2] ^ buf[3])) return STM32_ERR_UNKNOWN;
...
stm32_err_t stm32_crc_wrapper(const stm32_t* stm, uint32_t address, uint32_t length, uint32_t* crc)
{
    ...
    if (stm->cmd->crc != STM32_CMD_ERR) return stm32_crc_memory(stm, address, length, crc);
