...
static int flash_addr_to_page_floor(uint32_t addr)
{
    ...
    if (!is_addr_in_flash(addr)) return 0;
...
int flash_addr_to_page_ceil(uint32_t addr)
{
    ...
    if (!(addr >= stm->dev->fl_start && addr <= stm->dev->fl_end)) return 0;
...
int main(int argc, char* argv[])
{
    ...
    if (action == ACT_WRITE) {
        ...
        if (!use_stdinout) {
            if (!start_addr) start_addr = parser->base(p_st);
            if (start_addr) fprintf(diag, "Location     : %#08x\n", start_addr);

            if (!readwrite_len) readwrite_len = parser->size(p_st);
            fprintf(diag, "Size         : %u\n", readwrite_len);
        }
    }
    else {
    ...
    if (start_addr || readwrite_len) {
        ...
        if (!first_page && end == stm->dev->fl_end) num_pages = STM32_MASS_ERASE;
    }
    ...
    if (action == ACT_READ) {
        ...
        while (addr < end && offset < size) {
            ...
            if (parser->read(p_st, buffer, &len) != PARSER_ERR_OK) goto close;
        ...
        again:
            ...
            fprintf(diag, "\rWrote %saddress 0x%08x (%.2f%%) ", verify ? "and verified " : "", addr,
                    (100.0f / size) * offset);
            fflush(diag);
...
close:
    if (stm && exec_flag && ret == 0) {
        if (execute == 0) execute = stm->dev->fl_start;
        ...
        switch (c) {
            ...
            case 'S':
                if (spage || npages) {
                    ...
                    if (pLen != optarg && start_addr == 0) start_addr = 1;
                    if (*pLen == ':') {
                        pLen++;
            ...
            case 'F':
                ...
                if (port_opts.rx_frame_max == 0) port_opts.rx_frame_max = STM32_MAX_RX_FRAME;
                if (port_opts.tx_frame_max == 0) port_opts.tx_frame_max = STM32_MAX_TX_FRAME;
