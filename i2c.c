...
static port_err_t i2c_open(struct port_interface __unused* port, struct port_options __unused* ops)
{
...
static port_err_t i2c_open(struct port_interface* port, struct port_options* ops)
{
    struct i2c_priv* h;
    int fd, addr, ret;
    unsigned long funcs;
    ...
    if (strncmp(ops->device, "/dev/i2c-", strlen("/dev/i2c-"))) return PORT_ERR_NODEV;
...
static port_err_t i2c_read(struct port_interface* port, void* buf, size_t nbyte)
{
    ...
    if (ret != (int)nbyte) return PORT_ERR_UNKNOWN;
    return PORT_ERR_OK;
}

static port_err_t i2c_write(struct port_interface* port, void* buf, size_t nbyte)
{
    struct i2c_priv* h;
    int ret;
...
static port_err_t i2c_gpio(struct port_interface __unused* port, serial_gpio_t __unused n, int __unused level)
{
    return PORT_ERR_OK;
}
