...
static serial_t* serial_open(const char* device)
{
    ...
    h->fd = CreateFile(devName, GENERIC_READ | GENERIC_WRITE, 0, /* Exclusive access */
                       NULL,                                     /* No security */
                       OPEN_EXISTING, 0,                         /* No overlap */
                       NULL);

    if (devName != device) free(devName);

    if (h->fd == INVALID_HANDLE_VALUE) {
        if (GetLastError() == ERROR_FILE_NOT_FOUND) fprintf(stderr, "File not found: %s\n", device);
        free(h);
        return NULL;
    }
                               ...
                               const serial_parity_t parity, const serial_stopbit_t stopbit)
{
    switch (baud) {
        ...
        case SERIAL_BAUD_2500000:
            h->newtio.BaudRate = 2500000;
            break;
        case SERIAL_BAUD_3000000:
            h->newtio.BaudRate = 3000000;
            break;
        case SERIAL_BAUD_3500000:
            h->newtio.BaudRate = 3500000;
            break;
        case SERIAL_BAUD_4000000:
            h->newtio.BaudRate = 4000000;
            break;
    ...
    /* set the settings */
    serial_flush(h);
    if (!SetCommState(h->fd, &h->newtio)) return PORT_ERR_UNKNOWN;
...
static port_err_t serial_w32_open(struct port_interface* port, struct port_options* ops)
{
    ...
    /* 2. check options */
    if (ops->baudRate == SERIAL_BAUD_INVALID) return PORT_ERR_BAUD;
...
static port_err_t serial_w32_write(struct port_interface* port, void* buf, size_t nbyte)
{
    ...
    while (nbyte) {
        if (!WriteFile(h->fd, pos, nbyte, &r, NULL)) return PORT_ERR_UNKNOWN;
...
static port_err_t serial_w32_gpio(struct port_interface* port, serial_gpio_t n, int level)
{
    serial_t* h;
    int bit;
    ...
    switch (n) {
        ...
        case GPIO_BRK:
            ...
            if (EscapeCommFunction(h->fd, SETBREAK) == 0) return PORT_ERR_UNKNOWN;
            usleep(500000);
            if (EscapeCommFunction(h->fd, CLRBREAK) == 0) return PORT_ERR_UNKNOWN;
            return PORT_ERR_OK;
    ...
    /* handle RTS/DTR */
    if (EscapeCommFunction(h->fd, bit) == 0) return PORT_ERR_UNKNOWN;
