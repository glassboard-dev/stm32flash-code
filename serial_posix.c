...
static port_err_t serial_setup(serial_t* h, const serial_baud_t baud, const serial_bits_t bits,
                               const serial_parity_t parity, const serial_stopbit_t stopbit)
{
    speed_t port_baud;
    tcflag_t port_bits;
    ...
    if (cfsetispeed(&h->newtio, port_baud) || cfsetospeed(&h->newtio, port_baud)) {
        fprintf(stderr, "Failed to specify baud rate\n");
        return PORT_ERR_BAUD;
    }

    h->newtio.c_cflag |= port_parity | port_bits | port_stop | CLOCAL | CREAD;
    if (port_parity != 0) h->newtio.c_iflag |= INPCK;
    ...
    if (cfgetispeed(&settings) != port_baud || cfgetospeed(&settings) != port_baud) {
        fprintf(stderr, "Failed to set baud rate\n");
        return PORT_ERR_BAUD;
    }

    if (settings.c_iflag != h->newtio.c_iflag || settings.c_oflag != h->newtio.c_oflag ||
        settings.c_cflag != h->newtio.c_cflag || settings.c_lflag != h->newtio.c_lflag) {
        fprintf(stderr, "Failed to set terminal flags\n");
        return PORT_ERR_UNKNOWN;
    }

    snprintf(h->setup_str, sizeof(h->setup_str), "%u %d%c%d", serial_get_baud_int(baud), serial_get_bits_int(bits),
...
static port_err_t serial_posix_open(struct port_interface* port, struct port_options* ops)
{
    ...
    if (ops->baudRate == SERIAL_BAUD_KEEP) {
        ...
        if (h == NULL) return PORT_ERR_UNKNOWN;
        port->private = h;
        return PORT_ERR_OK;
    }

    /* 1. check options */
    if (ops->baudRate == SERIAL_BAUD_INVALID) return PORT_ERR_UNKNOWN;
    ...
    if (!isatty(h->fd)) fprintf(stderr, "Warning: Not a tty: %s\n", ops->device);

    /* 4. set options */
    ret = serial_setup(h, ops->baudRate, serial_get_bits(ops->serial_mode), serial_get_parity(ops->serial_mode),
...
static port_err_t serial_posix_read(struct port_interface* port, void* buf, size_t nbyte)
{
    serial_t* h;
    ssize_t r;
    ...
    while (nbyte) {
        ...
        if (r == 0) return PORT_ERR_TIMEDOUT;
        if (r < 0) return PORT_ERR_UNKNOWN;
...
static port_err_t serial_posix_write(struct port_interface* port, void* buf, size_t nbyte)
{
    serial_t* h;
    ssize_t r;
    ...
    while (nbyte) {
        ...
        if (r < 1) return PORT_ERR_UNKNOWN;
...
static port_err_t serial_posix_gpio(struct port_interface* port, serial_gpio_t n, int level)
{
    serial_t* h;
    int bit, lines;
    ...
    switch (n) {
        ...
        case GPIO_BRK:
            if (level == 0) return PORT_ERR_OK;
            if (tcsendbreak(h->fd, 1)) return PORT_ERR_UNKNOWN;
            return PORT_ERR_OK;
    ...
    /* handle RTS/DTR */
    if (ioctl(h->fd, TIOCMGET, &lines)) return PORT_ERR_UNKNOWN;
    lines = level ? lines | bit : lines & ~bit;
    if (ioctl(h->fd, TIOCMSET, &lines)) return PORT_ERR_UNKNOWN;
