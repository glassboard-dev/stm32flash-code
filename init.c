...
static int read_from(const char* filename, char* buf, size_t len)
{
    ...
    do {
        ...
        if (ret < 0) {
            if (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK) continue; /* try again */
...
#else
static int drive_gpio(int __unused n, int __unused level, struct gpio_list __unused** gpio_to_release)
{
...
static int gpio_bl_entry(struct port_interface* port, const char* seq)
{
    ...
    s = strchr(seq, ':');
    if (s == NULL) return gpio_sequence(port, seq, strlen(seq));
